...
function total_dist = actuwm(gam,steps,controller)
    ...
    %     gam = 0.001;  % gam = 0  means it's walking on the flat grounxsd
    end
...
%     k = 0
    ...
    F = @(t,y) original_controller(y,t,gam,a,tau,k)
    for i=1:steps
       [tout,yout] = ode45(@(t,y) f(t,y,F(t,y)),h,y0,opts); % Integrate for one stride
    ...
    % After gait has stabilized in a few steps
    % ------------------------------------------------------------------------
...
function F = original_controller(y,t,gam,a,tau,k)
    F = a*sin(2*pi/tau*t)+k*y(3);
end
